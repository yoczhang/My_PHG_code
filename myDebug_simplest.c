...
main(int argc, char *argv[])
{
    ...
    /*-------------------- myDebug1 --------------------*/
    /*
     * myDebug1 aiming to use the stiffnesss matrix A and rhs B(here just mark the matrix is A 
     * and rhs is B) built by the 'solver' in the function 'build_linear_system', and then solving 
     * the linear equations Ax=B according to my own way, the solutions is stored in the vec x.
     */
    printf("\n\n/*-------------------- myDebug1 --------------------*/\n\n");
    MAT *A_;
    VEC *B_,*x_;
    ...
    map_u_h=phgMapCreate(u_h,NULL);
    x_=phgMapCreateVec(map_u_h,1);
    phgVecDisassemble(x_);

    SOLVER *sol_;
    sol_=phgSolverMat2Solver(SOLVER_PCG,A_);
    
    phgVecCopy(solver->rhs,&(sol_->rhs));

    phgSolverVecSolve(sol_,TRUE,x_);

    int i_1=0;
    for(i_1=0;i_1<10;i_1++){
        printf("x_[%d]=%f ",i_1,x_->data[i_1]);
    }
...
//    return 0;
    ...
    /*-------------------- myDebug2 --------------------*/
    /*
     * myDebug2 aiming to printf the values of u_h->data
     */
    printf("\n\n/*-------------------- myDebug2 --------------------*/\n\n");
    FLOAT * val_u_h=u_h->data;
    int i_2=0; 
    for(i_2=0;i_2<10;i_2++){
        printf("val_u_h_[%d]=%f ",i_2,val_u_h[i_2]);
    }
